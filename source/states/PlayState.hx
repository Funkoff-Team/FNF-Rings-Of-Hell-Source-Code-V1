...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		switch (curStage)
		{
			...
			case 'crystalLake': new states.stages.CrystalLake(); //Bloodnight
			case 'theFungle': new states.stages.TheFungle(); //Sussus Hillus
		}
		...
		scoreTxt.setFormat(Paths.font("DungGeunMo.ttf"), 20, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		...
		switch(ClientPrefs.data.language.toLowerCase()){
			case 'ko-kr':
				botplayTxt = new FlxText(400, timeBar.y + 55, FlxG.width - 800, "봇 플레이", 32);
			default:
				botplayTxt = new FlxText(400, timeBar.y + 55, FlxG.width - 800, "BOTPLAY", 32);
		}
		botplayTxt.setFormat(Paths.font("DungGeunMo.ttf"), 32, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
...
/*	public function startVideo(name:String)
	...
	public dynamic function updateScore(miss:Bool = false)
	{
		...
		switch(ClientPrefs.data.language.toLowerCase()){
			case 'ko-kr':
				tempScore = '점수: ${songScore}'
				+ (!instakillOnMiss ? ' | 콤보 브레이크: ${songMisses}' : "")
				+ ' |정확도: ${str}';
			default:
	...
	override public function update(elapsed:Float)
	{
		...
		{
			if (controls.justPressed('debug_1')) {
			  CheatingState.isChartEditor = true;
				MusicBeatState.switchState(new CheatingState());
			}
			else if (controls.justPressed('debug_2')) {
			  CheatingState.isCharacterEditor = true;
				MusicBeatState.switchState(new CheatingState());
			}
		}
  	#end 
	...
	{
		var mult:Float = FlxMath.lerp(1, iconP1.scale.x, Math.exp(-elapsed * 30 * playbackRate));
		iconP1.scale.set(mult, mult);
		iconP1.updateHitbox();

		var mult:Float = FlxMath.lerp(1, iconP2.scale.x, Math.exp(-elapsed * 30 * playbackRate));
