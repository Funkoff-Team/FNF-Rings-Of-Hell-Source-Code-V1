...
class TitleState extends MusicBeatState
{
	...
	function startIntro()
	{
		...
		bg.loadGraphic(Paths.image('menus/EYX/titlemenu/menuBegin'));
		bg.scrollFactor.set(0.3, 0.3);
		bg.screenCenter();
		add(bg);

		logoBl = new FlxSprite(titleJSON.titlex, titleJSON.titley);
		logoBl.loadGraphic(Paths.image('menus/EYX/titlemenu/verylogo'));
		...
		gfDance.visible = !titleJSON.hide_gf;
		...
		titleText.frames = Paths.getSparrowAtlas('menus/EYX/titlemenu/pressEnter');
		var animFrames:Array<FlxFrame> = [];
		@:privateAccess {
			titleText.animation.findByPrefix(animFrames, "enter Loop");
		}
		
		if (animFrames.length > 0) {
			titleText.animation.addByPrefix('idle', "enter Loop", 24);
		...
		blackOut = new FlxSprite(0, 0).makeGraphic(FlxG.width, FlxG.height, FlxColor.BLACK);
		add(blackOut);
	...
	override function update(elapsed:Float)
	{
		...
		if (initialized && !transitioning && skippedIntro)
		{
			...
			if(pressedEnter)
			{
				...
				FlxG.sound.play(Paths.sound('enterLaugh'), 0.7);
		...
		if (!skippedIntro)
		{
			...
			else //Default! Edit this one!!
			{
				...
				FlxG.sound.play(Paths.sound('titleLaugh'), 1);
					FlxTween.tween(logoBl, {alpha: 1}, 0.4, {
						startDelay: 0.7,
					});
					...
					new FlxTimer().start(2, function(tmr:FlxTimer)
				{
				  FlxG.camera.flash(FlxColor.RED, 4);
					...
					{
						FlxG.sound.playMusic(Paths.music('titleMenu'), 0.7);
					});
