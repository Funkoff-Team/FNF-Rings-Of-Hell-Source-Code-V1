...
//We don't use Musicplayer anyway, so let's it'll just be here
...
class MusicPlayer extends FlxGroup 
{
	...
	override function update(elapsed:Float)
	{
		...
		if (paused && !wasPlaying)
			songTxt.text = 'PLAYING: ' + instance.songArray[FreeplayState.curSelected] + ' (PAUSED)';
		else
			songTxt.text = 'PLAYING: ' + instance.songArray[FreeplayState.curSelected];
 ...
 //Hide Freeplay texts and boxes if playingMusic is true
	...
	{
		var length:Int = instance.songArray[FreeplayState.curSelected].length;
